...
function TableContainerComponent({ rows, placeholderText, handleSearch, userType, getUpdatedData }) {
	const renderCell = (params) => {
		return (
			<div className="d-flex justify-content-between align-items-center" style={{ cursor: "pointer" }}>
				{params.row.profileImage ? (
					<img style={{ width: 40, height: 40, borderRadius: "50%" }} src={params.row.profileImage} alt="" />
				) : (
					<img
	...
	return (
		<div style={{ marginTop: "1rem" }}>
			...
			>
				<DataGrid
					rows={rows}
					columns={userType === "customer" ? customerColumns : businessesColumns}
					pageSize={5}
