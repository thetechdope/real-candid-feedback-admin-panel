	...
	const activateDeleteByEmail = async () => {
		try {
			...
			if (action === "delete") {
				...
				const FeedBackResponse = await axios.delete(`${baseUrl}/${userType}/delete/${mail}`);
		...
		<FormControlLabel
			control={
				<>
					...
					<div>
						{open === "delete" && <ModalComponent getAction={getAction} open="true" close={onHandleClose} msg="delete" />}
					</div>
				</>
			}
		/>
